...
x_vals = []
y_vals = []
name_of_grid_search = 'maze/'

x_vals = ["1_x", "0.8_x", "0.6_x", "0.4_x", "0.2_x","0_x"]
y_vals = ["1_y", "0.8_y", "0.6_y", "0.4_y", "0.2_y","0_y"]
...
for i in range(num_obst):
    ...
    rect = patches.Rectangle((xbound[0],ybound[0]),abs(xbound[1] - xbound[0]),abs(ybound[1] - ybound[0]), linewidth=1, zorder = 0, edgecolor='#567d46',facecolor='#567d46', fill=True)
    ax.add_patch(rect)
ax.scatter([START_POS[0]],[START_POS[1]],  [5], '#00FF00', zorder=3)
ax.scatter([END_POS[0]],[END_POS[1]],  [5], '#FF0000', zorder=3)
size = [18, 8, 10, 18, 8]
for j in range(5):
    for i in range(2):
        x = dedict[x_vals[j]][i]
...
plot_comparison = [(0.95, 1), (0.95, 0.8), (0.95, 0.6), (0.95, 0.4), (0.95, 0.2), (0.95, 0)]
...
labels=['λ=1', 'λ=0.8', 'λ=0.6', 'λ=0.4','λ=0.2','λ=0']
risks = risks[1:]
rets = rets[1:]
