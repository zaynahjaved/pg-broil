...
def generate_novice_demos(env, env_name, model_dir):
    checkpoint_min = 50
    checkpoint_max = 600
    checkpoint_step = 50
    checkpoints = []
...
def learn_reward(reward_network, optimizer, training_inputs, training_outputs, num_iter, l1_reg, checkpoint_dir):
    ...
    training_data_size = int(len(training_data)*.8)
    
    validation_data = training_data[training_data_size:]
    training_data = training_data[:training_data_size]
    
    best_validation_acc = -np.float('inf')
    stop_early = False
    ...
    for epoch in range(num_iter):
        ...
        for i in range(len(training_labels)):
            ...
            if i % 100 == 99:
                ...
                validation_accuracy = calc_accuracy(reward_net, validation_inputs, validation_outputs)
...
if __name__=="__main__":
    ...
    parser.add_argument('--models_dir', default = ".", help="path to directory that contains a models directory in which the checkpoint models for demos are stored.\
        In the models_dir, store the models in the form xxxxx.pt, where xxxxx is the checkpoint number (pad with 0s if needed)")
    ...
    min_snippet_length = 5 #min length of trajectory for training comparison, TODO: may need to change this for bigger envs, add if/else case
    ...
    # env = make_vec_env(env_id, 'mujoco', 1, seed,
    #                    wrapper_kwargs={
    #                        'clip_rewards':False
    #                    })
    env = gym.make(env_id)
    ...
    demonstrations, learning_returns, learning_rewards = generate_novice_demos(env, env_name, args.models_dir)
    ...
    learn_reward(reward_net, optimizer, training_obs, training_labels, num_iter, l1_reg, args.reward_net_path)
    #save reward network
    torch.save(reward_net.state_dict(), args.reward_net_path)
