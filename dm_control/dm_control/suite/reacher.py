...
class Physics(mujoco.Physics):
  ...
  def finger_to_obstacle(self):
    """Returns the vector from target to finger in global coordinates."""
    return (self.named.data.geom_xpos['obstacle', :2] -
            self.named.data.geom_xpos['finger', :2])

  def finger_to_obstacle_dist(self):
    """Returns the signed distance between the finger and target surface."""
    return np.linalg.norm(self.finger_to_obstacle())
...
class Reacher(base.Task):
  ...
  def __init__(self, target_size, random=None):
    ...
    self._obstacle_size = 0.05 # TODO Ashwin: make this configurable later
  ...
  def initialize_episode(self, physics):
    ...
    radius = self.random.uniform(.15, .20)
    physics.named.model.geom_pos['target', 'x'] = radius * np.sin(angle)
    physics.named.model.geom_pos['target', 'y'] = radius * np.cos(angle)

    # """Initialize obstacle"""
    physics.named.model.geom_size['obstacle', 0] = self._obstacle_size

    # Randomize obstacle position
    angle = self.random.uniform(0, 2 * np.pi)
    radius = self.random.uniform(.05, .1)
    physics.named.model.geom_pos['obstacle', 'x'] = radius * np.sin(angle)
    physics.named.model.geom_pos['obstacle', 'y'] = radius * np.cos(angle)
  ...
  def get_observation(self, physics):
    ...
    obs['to_obstacle'] = physics.finger_to_obstacle()
