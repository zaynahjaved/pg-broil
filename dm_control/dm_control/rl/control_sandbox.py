...
class Environment(dm_env.Environment):
  ...
  def reset(self):
    ...
    return flatten_observation(observation)['observations']
  ...
  def step(self, action):
    ...
    old_observation = self._task.get_observation(self._physics)
    if self._flat_observation:
      old_observation = flatten_observation(old_observation)
    old_constraint = self._task.get_constraint(self._physics)
    ...
    observation = self._task.get_observation(self._physics)
    constraint = self._task.get_constraint(self._physics)
    self.last_constraint = constraint

    if constraint and not old_constraint:
        # TODO Ashwin: may not always want to do this; ie. it's unclear whether the reacher task is velocity or force control rn
    ...
    return flatten_observation(observation)['observations'], reward, episode_over, {
