...
class ScratchItchEnv(AssistiveEnv):
    ...
    def get_reward(self, action, limb='shoulder'):
        tool_pos = self.tool.get_pos_orient(1)[0]
        if self.human.gender == 'male':
            if limb == 'shoulder':
                self.limb, length, radius = [self.human.right_shoulder, 0.279, 0.043]
            else:
                self.limb, length, radius = [self.human.right_elbow, 0.257, 0.033]
        else:
            if limb == 'shoulder':
                self.limb, length, radius = [self.human.right_shoulder, 0.264, 0.0355]
            else:
                self.limb, length, radius = [self.human.right_elbow, 0.234, 0.027]
